...
interface RewardsProps {
  ...
  onDeleteRedemption: (redemptionId: string) => Promise<boolean>;
}
...
export const Rewards = ({
  ...
  const [showDeleteRedemptionConfirm, setShowDeleteRedemptionConfirm] = useState<any>(null);
  ...
  const handleDeleteConfirm = () => {
    if (showDeleteConfirm) {
      onDeleteReward(showDeleteConfirm.id);
      setShowDeleteConfirm(null);
    }
  };

  const handleDeleteRedemptionClick = (redemption: any) => {
    setShowDeleteRedemptionConfirm(redemption);
  };

  const handleDeleteRedemptionConfirm = async () => {
    if (showDeleteRedemptionConfirm) {
      await onDeleteRedemption(showDeleteRedemptionConfirm.id);
                    ...
                    >
                      ...
                      <div className="absolute top-1 right-1 flex gap-1">
                        <Button
                          ...
                          className="w-7 h-7 bg-white hover:bg-amber-50 shadow-md hover:shadow-lg rounded-full border border-gray-200 hover:border-amber-300"
                        >
                        ...
                        <Button
                          ...
                          className="w-7 h-7 bg-white hover:bg-red-50 shadow-md hover:shadow-lg rounded-full border border-gray-200 hover:border-red-300"
                        >
                ...
                <AnimatePresence>
                  {state.redemptions.map((redemption, index) => (
                    ...
                    >
                      ...
                      <div className="text-amber-600 font-bold flex items-center gap-1 mr-2">
                        <span>-</span>
      ...
      <Dialog open={!!showDeleteConfirm} onOpenChange={() => setShowDeleteConfirm(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="text-center py-4">
            ...
            <h3 className="font-bold text-lg text-gray-800">{showDeleteConfirm?.name}</h3>
            <p className="text-gray-500 mt-2">删除后将无法恢复，确定要删除这个奖品吗？</p>
          </div>
          <div className="flex gap-2">
            <Button
              variant="outline"
      ...
      <Dialog open={!!showDeleteRedemptionConfirm} onOpenChange={() => setShowDeleteRedemptionConfirm(null)}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="text-center py-4">
            ...
            <h3 className="font-bold text-lg text-gray-800">{showDeleteRedemptionConfirm?.rewardName}</h3>
            <p className="text-gray-500 mt-2">撤销后将归还 {showDeleteRedemptionConfirm?.points} 积分，确定要撤销吗？</p>
            <div className="mt-4 flex items-center justify-center gap-2 text-green-600">
              <span>归还</span>
              <span className="text-2xl font-bold">+{showDeleteRedemptionConfirm?.points}</span>
              <span>⭐</span>
            </div>
          </div>
          <div className="flex gap-2">
